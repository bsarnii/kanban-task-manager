...
export const BoardMemberManagementStore = signalStore(
  ...
  withMethods((store, boardMembersDataService = inject(BoardMembersDataService), boardsStore = inject(BoardsStore)) => {
    const loadBoardMembers = rxMethod<string | null>(
      pipe(
        filter(Boolean),
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap((boardId) =>
          boardMembersDataService.getAll(boardId).pipe(
    ...
    const addMember = rxMethod<{ email: string; role: BoardMemberRole; callback: (success: boolean) => void }>(
      pipe(
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap(({ email, role, callback }) =>
          boardMembersDataService.add(boardsStore.activeBoardId()!, email, role).pipe(
            tap((boardMember) => {
    ...
    const updateMemberRole = rxMethod<{ id: string; role: BoardMemberRole }>(
      pipe(
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap(({ id, role }) =>
          boardMembersDataService.updateRole(boardsStore.activeBoardId()!, id, role).pipe(
            tap((updatedBoardMember) =>
              patchState(store, (state) => ({
                boardMembers: state.boardMembers.map((member) => (member.id === id ? updatedBoardMember : member)),
                loading: false,
              })),
            ),
            catchError(() => {
    ...
    const deleteMember = rxMethod<string>(
      pipe(
        filter(Boolean),
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap((id) =>
          boardMembersDataService.delete(boardsStore.activeBoardId()!, id).pipe(
            tap(() =>
              patchState(store, (state) => ({
