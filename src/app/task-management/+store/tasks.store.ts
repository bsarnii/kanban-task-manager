...
export const TasksStore = signalStore(
  ...
  withMethods((store, tasksDataService = inject(TasksDataService), boardsStore = inject(BoardsStore)) => {
    const loadTasks = rxMethod<string | null>(
      pipe(
        filter(Boolean),
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap((boardId) =>
          tasksDataService.getAll(boardId).pipe(
            tapResponse({
              next: (tasks) => {
    ...
    const addTask = rxMethod<TaskInputDto>(
      pipe(
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap((task) =>
          tasksDataService.create(boardsStore.activeBoardId()!, task).pipe(
            tapResponse({
              next: (task) =>
                patchState(store, (state) => ({ tasks: [...state.tasks, task], loading: false, loaded: true })),
              error: () => patchState(store, () => ({ loading: false })),
            }),
          ),
        ),
    ...
    const editTask = rxMethod<{ id: string; taskInput: Partial<TaskInputDto>; callback?: VoidFunction }>(
      pipe(
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap(({ id, taskInput, callback }) =>
          tasksDataService.update(boardsStore.activeBoardId()!, id, taskInput).pipe(
            tapResponse({
              next: (editedTask) => {
    ...
    const deleteTask = rxMethod<string>(
      pipe(
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap((taskId) =>
          tasksDataService.delete(boardsStore.activeBoardId()!, taskId).pipe(
            tapResponse({
              next: () =>
                patchState(store, (state) => ({
    ...
    const sortTasks = rxMethod<string[]>(
      pipe(
        tap(() => patchState(store, () => ({ loading: true }))),
        switchMap((taskIds) =>
          tasksDataService.sortTasks(boardsStore.activeBoardId()!, taskIds).pipe(
            tapResponse({
